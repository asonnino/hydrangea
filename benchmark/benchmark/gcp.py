...
class GCPInstanceManager:
    ...
    def create_instances(self, instances):
        ...
        try:
            for zone in ZONES:
                for i in range(instances):
                    config = {
                        ...
                        "metadata": {"items": [{"key": "ssh-keys", "value": ssh_key}]},
                    }

                    try:
                        request = compute_service.instances().insert(
                            project=self.PROJECT_ID, zone=zone, body=config
                        )
            ...
            self._wait()
            Print.heading(
                f"Successfully created {instances * len(self.settings.zones)} new instances"
            )
            print(
                f"Successfully created {instances * len(self.settings.zones)} "
                f"instances in {len(self.settings.zones)} zones."
            )
        except Exception as e:
    ...
    def delete_instances(self):
        try:
            ...
            for zone, client in self.clients.items():
                request = client.list(project=self.PROJECT_ID, zone=zone)
    ...
    def start_instances(self):
        try:
            for zone in self.settings.zones:
                request = self.compute_client.list(project=self.PROJECT_ID, zone=zone)
                for instance in request:
